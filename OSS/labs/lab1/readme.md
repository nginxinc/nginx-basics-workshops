...
|           NGINX OSS           |           Docker            |
| :---------------------------: | :-------------------------: |
| ![NGINX OSS](media/nginx.png) | ![Docker](media/docker.png) |

## Learning Objectives

By the end of the lab you will be able to:

- Introduction to the History and Architecture of NGINX
- Build an `NGINX Opensource Docker` image
- Build your Workshop environment with Docker Compose
...
|             NGINX             |                NGINX                |
| :---------------------------: | :---------------------------------: |
| ![NGINX OSS](media/nginx.png) | ![NGINX Logo](media/nginx-text.png) |
...
Another architectural concept in NGINX worth noting, is the `NGINX Master` process. The master process interacts with the Linux OS, controls the Workers, reads and validates config files before using them, writes to error and logging files, and performs other NGINX and Linux management tasks. It is considered the Control plane process, while the Workers are considered the Data plane processes. The `separation of Control functions from Data handling functions` is also very beneficial to handling high concurrency, high volume web traffic.
...
| :--------------------------------------------: | :-----------------------------: | :------------------------------: |
| ![Visual Studio](media/visual-studio-code.png) | ![NGINX Logo](media/docker.png) | ![Github Logo](media/github.png) |

1. **(Optional):** If you are using your own computer, create a new folder to hold all the Workshop materials. Then clone the Workshop's Github repository to this folder:
...
1. Inspect the Dockerfile, located in the `lab1/nginx-oss` folder. Notice the `FROM` directive uses the `NGINX Alpine` base image, and also the `RUN apk add` command, which installs additional tool libraries in the image. These tools are needed for copying/editing of files, and to run various tests while using the container in the exercises.
...
1. Verify your container is running by typing below command in a new terminal window:
